...
const PromptInput = ({ onSubmit, roomId, onLogUpdate }) => {
  ...
  const mockAgents = [
    {
      ...
      icon: "🔮",
      task: "Analyzing prompt and structuring requirements",
      duration: 3000,
    },
    {
      name: "GPT-4o",
      icon: "🧠",
      task: "Generating core application structure",
      duration: 4000,
    },
    {
      name: "Mistral Medium",
      icon: "⚡",
      task: "Optimizing code and adding features",
      duration: 3500,
    },
    {
      name: "Llama 3.3 70B",
      icon: "🦙",
      task: "Security review and finalization",
      duration: 2500,
    },
  ];
  ...
  const startMockDemo = async () => {
    ...
    for (let i = 0; i < mockAgents.length; i++) {
      ...
      for (let p = progressStart; p <= progressEnd; p += 2) {
        ...
        await new Promise((resolve) =>
          setTimeout(
            resolve,
            agent.duration / ((progressEnd - progressStart) / 2)
          )
        );
      }
    ...
    const finalLog = {
      timestamp: Date.now(),
      message:
        "🎉 All agents completed successfully! Files would be generated here.",
    };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const endpoint = isSecureMode
        ? `${API_BASE_URL}/ai/generate-secure`
            ...
            try {
              ...
              if (data.complete) {
                ...
                if (data.files && Array.isArray(data.files)) {
                  console.log(
                    `Creating ${data.files.length} files in IDE:`,
  ...
  return (
    <div
      className="h-full flex flex-col text-green-400"
          ...
          >
            ...
            <div className="space-y-2">
              ...
              <div className="ml-4 space-y-1">
                <div className="flex items-center justify-between">
                  <span className="text-green-400/80">
                    └─ GEMINI (Prompt Structuring)
                  </span>
                  <span
                    className={`${
                      currentAgent === "GEMINI_AGENT"
                        ? "text-yellow-400"
                        : "text-green-400/60"
                    }`}
                  >
              ...
              <div className="ml-4 space-y-1">
                <div className="flex items-center justify-between">
                  <span className="text-green-400/80">
                    └─ LLAMA 3.3 70B (Security)
                  </span>
                  <span
                    className={`${
                      currentAgent === "LLAMA_AGENT"
                        ? "text-yellow-400"
                        : "text-green-400/60"
                    }`}
                  >
                ...
                <div className="w-full bg-green-400/20 h-1">
                  <div
                    className="bg-purple-400 h-1 transition-all duration-300"
                    style={{
                      width: `${
                        currentAgent === "LLAMA_AGENT"
                          ? progress
            ...
            >
              <div className="flex items-center gap-2 mb-2">
                <div
                  className={`w-2 h-2 rounded-full animate-pulse ${
                    mockLoading ? "bg-purple-400" : "bg-green-400"
                  }`}
                ></div>
                <span
                  className={`text-xs font-bold ${
                    mockLoading ? "text-purple-400" : "text-green-400"
                  }`}
                >
              ...
              {mockLoading && (
                <div className="mb-3">
                  <div className="text-purple-400/80 text-xs mb-2">
                    Current Agent: {mockAgents[currentMockAgent]?.icon}{" "}
                    {mockAgents[currentMockAgent]?.name}
                  </div>
              ...
              <div
                className={`text-xs mb-2 ${
                  mockLoading ? "text-purple-400/80" : "text-green-400/80"
                }`}
              >
                &gt; {processStep}
              </div>
              <div
                className={`bg-black border p-2 ${
                  mockLoading ? "border-purple-400/20" : "border-green-400/20"
                }`}
              >
                <div className="flex justify-between text-xs mb-1">
                  <span
                    className={
                      mockLoading ? "text-purple-400/60" : "text-green-400/60"
                    }
                  >
                    Progress:
                  </span>
                ...
                <div
                  className={`w-full h-1 ${
                    mockLoading ? "bg-purple-400/20" : "bg-green-400/20"
                  }`}
                >
                  <div
