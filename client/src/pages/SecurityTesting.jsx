...
const SecurityTesting = () => {
  ...
  const API_BASE_URL = 'http://localhost:3003/api/security';
  ...
  const checkApiStatus = async () => {
    try {
      ...
      if (response.ok) {
        ...
        addTerminalNotification('Security API connected successfully', 'success');
      } else {
        setApiStatus('error');
  ...
  const fetchAvailableExploits = async () => {
    try {
      ...
      if (response.ok) {
        ...
        addTerminalNotification(`Loaded ${data.data.exploits.length} security exploits`, 'info');
      }
    } catch (error) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.success && data.data.files) {
          ...
          if (files.length > 0) {
            addTerminalNotification(`Found ${files.length} previously uploaded files`, 'info');
          }
        }
      }
    } catch (error) {
  ...
  const runSingleExploit = async (exploitName, target) => {
    if (!target) {
      addTerminalNotification('Please specify a target URL or upload files', 'error');
      return;
    }
    ...
    setScanResults([]);
    addTerminalNotification('Cleared previous results - starting fresh scan', 'info');
    addTerminalNotification(`Starting ${exploitName} scan on ${target}`, 'info');
    ...
    try {
      ...
      if (response.ok && data.success) {
        addTerminalNotification(`${exploitName} scan completed - ${data.data.vulnerabilities?.length || 0} vulnerabilities found`, 'success');
        fetchScanResults();
      } else {
        addTerminalNotification(`${exploitName} scan failed: ${data.error}`, 'error');
      }
    } catch (error) {
      addTerminalNotification(`${exploitName} scan error: ${error.message}`, 'error');
    } finally {
      setActiveScans(prev => {
  ...
  const runSecuritySuite = async () => {
    ...
    if (selectedExploits.length === 0) {
      addTerminalNotification('Please select at least one exploit to run', 'error');
      return;
    }
    ...
    try {
      ...
      if (uploadedFiles.length > 0) {
        ...
        addTerminalNotification(`Starting file scan with ${selectedExploits.length} exploits on ${uploadedFiles.length} files`, 'info');
        ...
        addTerminalNotification(`Starting URL scan with ${selectedExploits.length} exploits on ${selectedTarget}`, 'info');
      ...
      if (response.ok && data.success) {
        const totalVulns = data.data.results?.reduce((sum, result) =>
          sum + (result.vulnerabilities?.length || 0), 0) || 0;
        addTerminalNotification(`Security suite completed - ${totalVulns} total vulnerabilities found`, 'success');
        fetchScanResults();
      } else {
        addTerminalNotification(`Security suite failed: ${data.error}`, 'error');
      }
    } catch (error) {
      addTerminalNotification(`Security suite error: ${error.message}`, 'error');
    } finally {
      setActiveScans(prev => {
        const newSet = new Set(prev);
  ...
  const handleFileUpload = async (files) => {
    ...
    try {
      ...
      if (response.ok && data.success) {
        ...
        setScanResults([]);
        addTerminalNotification(`Uploaded ${newFiles.length} file(s) for security testing`, 'success');
        addTerminalNotification('Previous results cleared - ready for new scan', 'info');
      } else {
  ...
  const clearAllResults = async () => {
    try {
      ...
      if (response.ok) {
        setScanResults([]);
        addTerminalNotification('All scan results cleared from server', 'success');
      } else {
        addTerminalNotification('Failed to clear results from server', 'error');
      }
    } catch (error) {
      addTerminalNotification(`Error clearing results: ${error.message}`, 'error');
    }
  };
  ...
  const getStatusColor = (status) => {
    switch (status) {
      ...
      case "pending": return "text-green-400/60";
  ...
  const getSeverityColor = (severity) => {
    switch (severity) {
      ...
      default: return "text-green-400/60 bg-green-400/10";
    }
  };
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2 text-xs">
              ...
              <span className="text-green-400/80">
                {apiStatus === 'connected' ? 'API_CONNECTED' :
                 apiStatus === 'error' ? 'API_ERROR' : 'CONNECTING...'}
              </span>
            </div>
            <button
      ...
      <div className="flex h-[calc(100vh-120px)]">
            ...
            <div className="space-y-3">
              <div className="p-3 border border-green-400/20 bg-green-400/5">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span className="text-green-400 font-bold">
                    {Math.round((scanResults.filter(r => r.vulnerabilities?.length === 0).length / Math.max(scanResults.length, 1)) * 100)}%
                  </span>
                </div>
                <div className="w-full bg-green-400/20 h-2">
                  <div
                    ...
                    style={{
                      width: `${Math.round((scanResults.filter(r => r.vulnerabilities?.length === 0).length / Math.max(scanResults.length, 1)) * 100)}%`
                    }}
                  ></div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-2">
                <div className="p-2 border border-red-400/30 bg-red-400/5">
                  ...
                  <div className="text-red-400 font-bold text-lg">{activeScans.size}</div>
                </div>
                <div className="p-2 border border-yellow-400/30 bg-yellow-400/5">
                  <div className="text-yellow-400 text-xs">Exploits</div>
                  <div className="text-yellow-400 font-bold text-lg">{availableExploits.length}</div>
                </div>
              </div>

              <div className="p-3 border border-green-400/20 bg-green-400/5">
                <div className="text-green-400/80 text-xs mb-1">Total Scans</div>
                <div className="text-green-400 text-sm">{scanResults.length}</div>
              </div>

              <div className="p-3 border border-green-400/20 bg-green-400/5">
                <div className="text-green-400/80 text-xs mb-1">Uploaded Files</div>
                <div className="text-green-400 text-sm">{uploadedFiles.length}</div>
              </div>
            </div>
          </div>
        ...
        <div className="flex-1 p-6">
          ...
          {activeTab === 'dashboard' && (
            <div className="space-y-6">
              <div className="flex items-center justify-between">
                <h2 className="text-green-400 text-lg font-bold">SECURITY_DASHBOARD</h2>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                {scanResults.slice(0, 4).map((result, index) => (
                  <div key={result.scanId || index} className="border border-green-400/30 bg-green-400/5 p-4">
                    <div className="flex items-center justify-between mb-3">
                      <span className="text-green-400 font-bold text-sm">{result.exploitName}</span>
                      <div className={`px-2 py-1 text-xs rounded ${getSeverityColor(
                        result.vulnerabilities?.length > 5 ? 'critical' :
                        result.vulnerabilities?.length > 2 ? 'high' :
                        result.vulnerabilities?.length > 0 ? 'medium' : 'low'
                      )}`}>
                    ...
                    <div className="space-y-1 text-xs">
                      <div className="flex justify-between">
                        ...
                        <span className="text-green-400 truncate ml-2">{result.target}</span>
                      </div>
                      <div className="flex justify-between">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                <div className="border border-green-400/30 bg-green-400/5 p-4">
                  ...
                  <div className="space-y-2 text-xs">
                    <div className="flex justify-between">
                      ...
                      <span className="text-green-400">
                        {scanResults.filter(r => r.exploitName === 'sqlInjection').length} TESTS
                      </span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-green-400/80">Vulnerabilities:</span>
                      <span className={
                        scanResults.filter(r => r.exploitName === 'sqlInjection' && r.vulnerabilities?.length > 0).length > 0
                ...
                <div className="border border-green-400/30 bg-green-400/5 p-4">
                  ...
                  <div className="space-y-2 text-xs">
                    <div className="flex justify-between">
                      ...
                      <span className="text-green-400">
                        {scanResults.filter(r => r.exploitName === 'networkScanner').length} TESTS
                      </span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-green-400/80">SSL Analysis:</span>
                      <span className="text-green-400">
                        {scanResults.filter(r => r.exploitName === 'sslAnalyzer').length} TESTS
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="border border-green-400/30 bg-green-400/5 p-4">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <span className="text-green-400 font-bold text-sm">WEB_SECURITY</span>
                  </div>
                  <div className="space-y-2 text-xs">
                    <div className="flex justify-between">
                      <span className="text-green-400/80">XSS Tests:</span>
                      <span className="text-green-400">
                        {scanResults.filter(r => r.exploitName === 'xss').length} TESTS
                      </span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-green-400/80">CSRF Tests:</span>
                      <span className="text-green-400">
                        {scanResults.filter(r => r.exploitName === 'csrf').length} TESTS
                      </span>
                    </div>
                  </div>
                </div>
          ...
          {activeTab === 'exploits' && (
            <div className="space-y-6">
              <div className="flex items-center justify-between">
                <h2 className="text-green-400 text-lg font-bold">SECURITY_EXPLOITS</h2>
                <div className="flex gap-2">
                  <input
                    type="text"
                  ...
                  <button
                    onClick={() => {
                      ...
                      addTerminalNotification('Selected all exploits for testing', 'info');
                    }}
              ...
              {uploadedFiles.length > 0 && selectedExploits.length === 0 && (
                <div className="border border-yellow-400/30 bg-yellow-400/5 p-4 mb-4">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="text-yellow-400 font-bold text-sm">SELECT EXPLOITS TO TEST YOUR FILES</span>
                  </div>
                  <div className="text-yellow-400/80 text-xs">
                    You have {uploadedFiles.length} file(s) uploaded. Click on the exploits below to select them for testing, then run the scan.
                  </div>
                  <div className="text-yellow-400/60 text-xs mt-2">
                    💡 New scans will automatically clear previous results to show only current file vulnerabilities.
                  </div>
                </div>
              )}
              ...
              {selectedTarget && selectedExploits.length === 0 && (
                <div className="border border-blue-400/30 bg-blue-400/5 p-4 mb-4">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="text-blue-400 font-bold text-sm">TARGET SET - SELECT EXPLOITS</span>
                  </div>
                  <div className="text-blue-400/80 text-xs">
                    Target URL: {selectedTarget}. Select exploits below to test this target.
                  </div>
                </div>
              )}
              ...
              {!selectedTarget && uploadedFiles.length === 0 && (
                <div className="border border-green-400/30 bg-green-400/5 p-4 mb-4">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="text-green-400 font-bold text-sm">READY FOR TESTING</span>
                  </div>
                  <div className="text-green-400/80 text-xs">
                    Enter a target URL above or upload files in the FILE_UPLOAD tab, then select exploits to run.
                  </div>
                </div>
              )}
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-4">
                {availableExploits.map((exploit) => (
                  ...
                  >
                    <div className="flex items-center justify-between mb-3">
                      <span className="text-green-400 font-bold text-sm">{exploit.displayName}</span>
                      <div className={`px-2 py-1 text-xs rounded ${getSeverityColor(exploit.severity)}`}>
                        {exploit.severity.toUpperCase()}
                      </div>
                    </div>
                    <div className="space-y-1 text-xs mb-3">
                      <div className="text-green-400/80">Category: {exploit.category}</div>
                      <div className="text-green-400/80">Type: {exploit.name}</div>
                    </div>
                    <div className="flex gap-2">
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                          runSingleExploit(exploit.name.replace(/([A-Z])/g, '-$1').toLowerCase().replace(/^-/, ''), selectedTarget);
                        }}
                      ...
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                          // Show exploit info
                          addTerminalNotification(`${exploit.displayName}: ${exploit.category} security test`, 'info');
                        }}
              ...
              {selectedExploits.length > 0 && (
                <div className="border border-green-400/30 bg-green-400/5 p-4">
                  <div className="flex items-center justify-between mb-3">
                    <span className="text-green-400 font-bold">Selected Exploits ({selectedExploits.length})</span>
                    <button
                      onClick={runSecuritySuite}
                      disabled={(!selectedTarget && uploadedFiles.length === 0) || activeScans.size > 0}
                      className="px-4 py-2 border border-green-400/30 hover:bg-green-400/10 text-green-400 text-xs transition-all disabled:opacity-50"
                    >
                  ...
                  {(!selectedTarget && uploadedFiles.length === 0) && (
                    <div className="mb-3 p-2 bg-red-400/10 border border-red-400/30 text-red-400 text-xs">
                      ⚠️ Please set a target URL above or upload files in FILE_UPLOAD tab before running exploits.
                    </div>
                  )}

                  <div className="mb-3 text-xs text-green-400/80">
                    {uploadedFiles.length > 0 && `Testing ${uploadedFiles.length} uploaded file(s)`}
                    {selectedTarget && `Testing target: ${selectedTarget}`}
                  </div>
                  <div className="flex flex-wrap gap-2">
                    {selectedExploits.map(exploit => (
                      <span key={exploit} className="px-2 py-1 bg-green-400/20 text-green-400 text-xs">
                        {availableExploits.find(e => e.name === exploit)?.displayName || exploit}
                      </span>
                    ))}
                  </div>
                </div>
          ...
          {activeTab === 'files' && (
            <div className="space-y-6">
              <div className="flex items-center justify-between">
                <h2 className="text-green-400 text-lg font-bold">FILE_UPLOAD_TESTING</h2>
                <button
              ...
              >
                ...
                <div className="text-green-400 mb-2">Drop files here or click to browse</div>
                <div className="text-green-400/60 text-xs">
                  Supported: .js, .php, .html, .css, .json, .xml, .sql, .py, .java, .cpp
                </div>
              </div>
              ...
              {uploadedFiles.length > 0 && (
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    <h3 className="text-green-400 font-bold">UPLOADED_FILES ({uploadedFiles.length})</h3>
                    <button
                      onClick={async () => {
                        // Delete all files from server
                        for (const file of uploadedFiles) {
                          try {
                            await fetch(`${API_BASE_URL}/files/${file.id}`, { method: 'DELETE' });
                          } catch (error) {
                            console.error('Error deleting file:', error);
                          }
                        }
                        setUploadedFiles([]);
                        addTerminalNotification('Cleared all uploaded files', 'info');
                      }}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {uploadedFiles.map((file) => (
                      <div key={file.id} className="border border-green-400/30 bg-green-400/5 p-3">
                        <div className="flex items-center justify-between mb-2">
                        ...
                        <div className="space-y-1 text-xs">
                          ...
                          <div className="text-green-400/80">Size: {formatFileSize(file.size)}</div>
                          <div className="text-green-400/80">Type: {file.type || 'Unknown'}</div>
                  ...
                  <div className="border border-green-400/30 bg-green-400/5 p-4">
                    <div className="flex items-center justify-between mb-3">
                      <span className="text-green-400 font-bold">FILE_TESTING_OPTIONS</span>
                      <button
                        onClick={() => {
                          if (selectedExploits.length === 0) {
                            addTerminalNotification('Please select exploits first in the EXPLOITS tab', 'error');
                        ...
                        className="px-4 py-2 border border-green-400/30 hover:bg-green-400/10 text-green-400 text-xs transition-all disabled:opacity-50"
                      >
                        {activeScans.size > 0 ? '[SCANNING_FILES...]' : '[TEST_FILES]'}
                      </button>
                    </div>
                    <div className="text-xs text-green-400/80">
                      Files will be analyzed for common vulnerabilities including:
                      XSS, SQL Injection, Path Traversal, Code Injection, and more.
                    </div>
                  </div>
                </div>
              )}
          ...
          {activeTab === 'results' && (
            <div className="space-y-6">
              <div className="flex items-center justify-between">
                <h2 className="text-green-400 text-lg font-bold">SCAN_RESULTS</h2>
                <div className="flex gap-2">
                  <button
                    onClick={fetchScanResults}
                    className="px-4 py-2 border border-green-400/30 hover:bg-green-400/10 text-green-400 text-xs transition-all"
                  ...
                  <button
                    onClick={() => {
                      ...
                      addTerminalNotification('Exported scan results to CSV', 'success');
                    }}
              ...
              {scanResults.length === 0 ? (
                <div className="border border-green-400/30 bg-green-400/5 p-8 text-center">
                  ...
                  <div className="text-green-400/80">No scan results available</div>
                  ...
                  <div className="text-green-400/40 text-xs mt-3 border-t border-green-400/20 pt-3">
                    🔄 Results are isolated per scan session - each new scan clears previous results
                  </div>
                </div>
              ) : (
                <div className="space-y-4">
                  <div className="border border-blue-400/30 bg-blue-400/5 p-3 mb-4">
                    <div className="flex items-center gap-2 text-xs text-blue-400">
                      ...
                      <span>Showing results for current scan session only - previous results automatically cleared</span>
                    </div>
                  </div>
                  {scanResults.map((result, index) => (
                    <div key={result.scanId || index} className="border border-green-400/30 bg-green-400/5 p-4">
                      <div className="flex items-center justify-between mb-3">
                        <div className="flex items-center gap-2">
                          <span className="text-green-400 font-bold">{result.exploitName}</span>
                          <div className={`px-2 py-1 text-xs rounded ${getSeverityColor(
                            result.vulnerabilities?.length > 5 ? 'critical' :
                            result.vulnerabilities?.length > 2 ? 'high' :
                            result.vulnerabilities?.length > 0 ? 'medium' : 'low'
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-3">
                        <div className="space-y-1 text-xs">
                          ...
                          <div className="flex justify-between">
                            ...
                            <span className="text-green-400">{result.duration || 'N/A'}ms</span>
                          </div>
                        </div>
                        <div className="space-y-1 text-xs">
                          <div className="flex justify-between">
                            <span className="text-green-400/80">Scan ID:</span>
                            <span className="text-green-400 font-mono">{result.scanId || 'N/A'}</span>
                          </div>
                          <div className="flex justify-between">
                      ...
                      {result.vulnerabilities && result.vulnerabilities.length > 0 && (
                        <div className="border-t border-green-400/20 pt-3">
                          <div className="text-green-400/80 text-xs mb-2">VULNERABILITIES FOUND:</div>
                          <div className="space-y-2">
                            {result.vulnerabilities.slice(0, 3).map((vuln, vIndex) => (
                              <div key={vIndex} className="bg-red-400/10 border border-red-400/30 p-2">
                                <div className="flex items-center justify-between mb-1">
                                  <span className="text-red-400 text-xs font-bold">{vuln.type}</span>
                                  <span className={`px-1 py-0.5 text-xs rounded ${getSeverityColor(vuln.severity)}`}>
                                    {vuln.severity?.toUpperCase()}
                                  </span>
                                </div>
                                <div className="text-xs space-y-1">
                                  {vuln.location && (
                                    <div className="text-green-400/80">Location: {vuln.location}</div>
                                  )}
                                  {vuln.payload && (
                                    <div className="text-green-400/80">Payload: <code className="text-green-400">{vuln.payload}</code></div>
                                  )}
                                  {vuln.evidence && (
                                    <div className="text-green-400/80">Evidence: {vuln.evidence}</div>
                                  )}
                                </div>
                              </div>
                            ))}
                            {result.vulnerabilities.length > 3 && (
                              <div className="text-green-400/60 text-xs text-center">
                                ... and {result.vulnerabilities.length - 3} more vulnerabilities
                              </div>
                            )}
                          </div>
                        </div>
          ...
          {activeTab === 'config' && (
            <div className="space-y-6">
              <h2 className="text-green-400 text-lg font-bold">SECURITY_CONFIGURATION</h2>

              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div className="border border-green-400/30 bg-green-400/5 p-4">
                  <h3 className="text-green-400 font-bold mb-4">SCAN_SETTINGS</h3>
                  <div className="space-y-4">
                    <div>
                      <label className="block text-green-400/80 text-xs mb-2">Timeout (ms)</label>
                      <input
                        type="number"
                    ...
                    <div>
                      <label className="block text-green-400/80 text-xs mb-2">Max Depth</label>
                      <input
                        type="number"
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      />
                      <label htmlFor="aggressive" className="text-green-400/80 text-xs">Aggressive Scanning</label>
                    </div>
                  </div>
                </div>
                ...
                <div className="border border-green-400/30 bg-green-400/5 p-4">
                  ...
                  <div className="space-y-2 text-xs">
                    ...
                    <div className="flex justify-between">
                      <span className="text-green-400/80">Exploits Loaded:</span>
                      <span className="text-green-400">{availableExploits.length}</span>
                    </div>
                    <div className="flex justify-between">
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="text-green-400">{scanResults.length}</span>
                    </div>
                  </div>
                  <button
                    onClick={checkApiStatus}
